...
class DocumentPedomanController extends Controller
{
    /**
    ...
    {
        // Ambil semua unit (atau filter sesuai kebutuhan, misal hanya prodi atau fakultas)
        // Disini saya ambil semua unit karena 'pedoman' mungkin berlaku umum
        $units = UnitDokument::whereIn('jenis', ['Fakultas', 'Universitas', 'Pasca Sarjana'])->orderBy('nama')->get();
        return view('admin.document-pedoman.index', compact('units'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $data = [
                ...
                'status' => 'acc', // Default status, bisa disesuaikan
            ];
            ...
            if ($request->hasFile('file')) {
                ...
                $path = $file->storeAs('documents/pedoman', $filename, 'public');
                $data['path'] = $path;
            }

            DocumentPedoman::create($data);

            return response()->json([
                'status' => true,
                'message' => 'Dokumen Pedoman berhasil ditambahkan.'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $document = DocumentPedoman::findOrFail($id);
            ...
            return response()->json([
                'status' => true,
                'message' => 'Dokumen Pedoman berhasil diperbarui.'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
    ...
    public function destroy($id)
    {
        try {
            ...
            return response()->json([
                'status' => true,
                'message' => 'Dokumen Pedoman berhasil dihapus.'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
