<div class="td-sidebar">
    ...
    <div class="widget widget_catagory">
        ...
        <ul class="catagory-items">
            @foreach ($category as $item)
                <li><a href="{{ route('news.index', ['category' => $item->name]) }}"><i class="fas fa-caret-right"></i>
                        {{ $item->name }}
                        <span>{{ $item->newsCategory->count() }}</span></a>
                </li>
            @endforeach
        </ul>
    </div>
                ...
                <li>
                    <div class="media">
                        <div class="media-left">
                            <img src={{ $item->image ? asset('storage/image-news') . '/' . rawurlencode($item->image) : asset('assets/img/widget/1.png') }}
    ...
    <div class="widget widget_tag_cloud mb-0">
        ...
        <div class="tagcloud">
            @foreach ($category as $item)
                <a href="{{ route('news.index', ['category' => $item->name]) }}">{{ $item->name }}</a>
            @endforeach
        </div>
    </div>
    <!-- Tags End -->
