...
@push('scripts')
    <script>
        $(document).on('click', '#new-record-button', function() {
            location.href = "{{ route('admin.ami-auditor-decree.add') }}";
        });
        ...
        $(document).on('submit', '.form-delete-record', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.ami-auditor-decree.delete') }}",
    ...
    <script>
        var dataTable = initDataTables('table-1', 'loader-ami-auditor-decree', 'card-ami-auditor-decree', 'new-record-button', false,
            'SK Auditor AMI', "{{ route('admin.ami-auditor-decree.data') }}",
            [{
                    data: "ami_period",
                    name: "ami_period",
