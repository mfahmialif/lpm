...
@section('title', 'Dokumen Keputusan Rektor')
@section('content')
...
<div class="offcanvas offcanvas-end" id="new-record">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title">Tambah Dokumen Keputusan Rektor</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body flex-grow-1">
        <form class="record pt-0 row g-2" id="form-new-record" action="{{ route('admin.document-keputusan-rektor.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @include('admin.document-keputusan-rektor.form')
            <div class="col-sm-12">
...
<div class="offcanvas offcanvas-end" id="edit-record">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title">Edit Dokumen Keputusan Rektor</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body flex-grow-1">
...
<script>
    ...
    $(document).on('click', '#new-record-button', function() {
        ...
        $('#form-new-record .prodi-container').hide();
        $('#form-new-record [name="prodi_id"]').val('');
    ...
    $(document).on('click', '.edit-record-button', function() {
        ...
        const unit_dokument_id = $(this).data('unit_dokument_id');
        const prodi_id = $(this).data('prodi_id');

        $('#form-edit-record').attr('action', "{{ url('admin/document-keputusan-rektor/update') }}/" + id);
        $('#form-edit-record [name="id"]').val(id);
        $('#form-edit-record [name="unit_dokument_id"]').val(unit_dokument_id).trigger('change');
        ...
        $('#form-edit-record [name="perihal"]').val($(this).data('perihal'));
        $('#form-edit-record [name="yang_mengeluarkan"]').val($(this).data('yang_mengeluarkan'));
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ url('admin/document-keputusan-rektor/destroy') }}/" + id,
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
    ...
    // Show/hide prodi select based on unit jenis
    $(document).on('change', '.unit-select', function() {
        var form = $(this).closest('form');
        var selectedOption = $(this).find('option:selected');
        var jenis = selectedOption.data('jenis');

        if (jenis && jenis.toLowerCase() === 'prodi') {
            form.find('.prodi-container').show();
        } else {
            form.find('.prodi-container').hide();
            form.find('[name="prodi_id"]').val('');
        }
    });
</script>
...
<script>
    var dataTable = initDataTables('table-1', 'loader-document', 'card-document', 'new-record-button', false,
        'Dokumen Keputusan Rektor', "{{ route('admin.document-keputusan-rektor.data') }}",
        [{
                data: "unit_nama",
                name: "unit_nama",
                ...
                render: function(data, type, row) {
                    return `
                        <button type="button" class="btn btn-sm btn-warning edit-record-button me-1" 
                            data-id="${row.id}" 
                            data-unit_dokument_id="${row.unit_dokument_id || ''}"
