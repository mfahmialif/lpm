        ...
        <div class="isi-body">
            @if($indicator)
                ...
                <form id="form-selection">
                        ...
                        @if($scores->count() > 0)
                            ...
                            <label class="score-checkbox-item {{ in_array($score->id, $selectedScoreIds) ? 'checked' : '' }}">
                                <input type="checkbox" name="score_ids[]" value="{{ $score->id }}" class="score-checkbox"
                                       ...
                                       {{ (isset($canInputScore) && !$canInputScore) ? 'disabled' : '' }}>
                    ...
                    @if($scores->count() > 0)
                        @if(isset($canInputScore) && $canInputScore)
                        ...
                        <div class="alert alert-warning mt-3 mb-0 d-flex align-items-center" role="alert">
                            ...
                            <div>
                                <strong>Akses Terbatas:</strong> Role Unit pada mode {{ ucwords(\App\Http\Services\Helper::getAmiMode()) }} tidak dapat mengisi skor.
                            </div>
                        </div>
                        @endif
                    @endif
...
<script>
    $(document).ready(function() {
        ...
        $('#form-selection').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("admin.ami-auditor-assessment.storeIsiIndikator", $amiAuditorAssessment->id) }}',
                data: $(this).serialize(),
