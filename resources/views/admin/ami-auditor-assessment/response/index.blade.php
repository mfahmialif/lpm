...
@section('title', 'Respon Asesmen Auditor')
@push('styles')
<style>
    .chat-page-wrapper {
        max-width: 900px;
...
<div class="chat-page-wrapper">
    ...
    <div class="chat-card">
                ...
                <div class="chat-header-info">
                    ...
                    <div class="chat-header-meta">
                        <span class="meta-tag tag-period">
                            ...
                            {{ $amiAuditorAssessment->amiPeriod->year ?? '-' }}
                        </span>
                        <span class="meta-tag tag-unit">
                            <i class="ti ti-building"></i>
                            {{ $amiAuditorAssessment->prodiUnit->nama ?? '-' }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="chat-footer">
            @if(isset($canInputResponse) && $canInputResponse)
            <form id="chat-form">
                @csrf
            ...
            <div class="alert alert-warning mb-0 d-flex align-items-center" role="alert">
                ...
                <div>
                    <strong>Akses Terbatas:</strong> Role Unit pada mode {{ ucwords(\App\Http\Services\Helper::getAmiMode()) }} tidak dapat mengirim respon.
                </div>
            </div>
            @endif
        </div>
...
<script>
    $(document).ready(function() {
        ...
        chatForm.on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST'
                , url: '{{ route("admin.ami-auditor-assessment.storeResponse", $amiAuditorAssessment->id) }}'
                , data: formData
                , contentType: false
                , processData: false
        ...
        function loadMessages() {
            $.ajax({
                type: 'GET',
                url: '{{ route("admin.ami-auditor-assessment.getResponses", $amiAuditorAssessment->id) }}',
                success: function(response) {
        ...
        $(document).on('click', '.btn-delete', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'DELETE'
                        , url: '{{ url("admin/ami-auditor-assessment/response/message") }}/' + msgId
                        , data: {
                            _token: '{{ csrf_token() }}'
                        }
                        , success: function(response) {
