...
@section('title', 'Edit | Asesmen Diri AMI')
@section('content')
<div class="mb-6">
    <div class="card h-100">
        <div class="card-header d-flex justify-content-between">
            <div class="card-title mb-0">
                <h5 class="mb-1">Form Edit Asesmen Diri AMI</h5>
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="MonthlyCampaign">
                    <a href="{{ route('admin.ami-self-assessment.index') }}" class="dropdown-item">Back</a>
                </div>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.ami-self-assessment.update', ['amiSelfAssessment' => $amiSelfAssessment]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                @include('admin.ami-self-assessment.form', ['mode' => 'edit'])
            </form>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        var data = @json($amiSelfAssessment);
        $('[name="ami_period_id"]').val(data.ami_period_id).trigger('change');
        $('[name="prodi_unit_id"]').val(data.prodi_unit_id).trigger('change');
