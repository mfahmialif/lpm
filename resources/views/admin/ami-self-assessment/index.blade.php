...
@section('title', 'Asesmen Diri AMI')
@section('content')
    <div class="card" id="card-ami-self-assessment">
...
@push('scripts')
    <script>
        $(document).on('click', '#new-record-button', function() {
            location.href = "{{ route('admin.ami-self-assessment.add') }}";
        });
        ...
        $(document).on('submit', '.form-delete-record', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.ami-self-assessment.delete') }}",
    ...
    <script>
        var dataTable = initDataTables('table-1', 'loader-ami-self-assessment', 'card-ami-self-assessment', 'new-record-button', false,
            'Asesmen Diri AMI', "{{ route('admin.ami-self-assessment.data') }}",
            [{
                    data: "ami_period",
                    name: "ami_period",
