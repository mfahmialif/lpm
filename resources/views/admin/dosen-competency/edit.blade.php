...
@section('title', 'Edit Kompetensi Dosen')
@section('content')
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Edit Kompetensi Dosen</h5>
        <a href="{{ route('admin.dosen-competency.index') }}" class="btn btn-secondary">
            <i class="ti ti-arrow-left me-1"></i> Kembali
        </a>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.dosen-competency.update', $data->id) }}" method="POST" class="row g-3">
            @csrf
            @method('PUT')
...
<script>
    $(document).ready(function() {
        ...
        if (initialProdiId) {
            loadCompetencies(initialProdiId, "{{ old('prodi_competency_id', $data->prodi_competency_id) }}");
        }
    });
</script>
@endpush