...
@section('content')
    ...
    <div class="offcanvas offcanvas-end" id="new-record">
        ...
        <div class="offcanvas-body flex-grow-1">
            <form class="record pt-0 row g-2" id="form-new-record" action="{{ route('admin.accreditation.store') }}" method="POST">
                @csrf
                @include('admin.accreditation.form')
                <div class="col-sm-12">
    ...
    <div class="offcanvas offcanvas-end" id="edit-record">
        ...
        <div class="offcanvas-body flex-grow-1">
            <form class="record pt-0 row g-2" id="form-edit-record" action="{{ route('admin.accreditation.update') }}"
                method="POST">
                @csrf
                @method('PUT')
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.edit-record-button', function() {
            ...
            const resultDescription = $(this).data('result-description');
            ...
            $('#form-edit-record [name="status"]').val(status);
            $('#form-edit-record [name="result"]').val(result);
            $('#form-edit-record [name="result_description"]').val(resultDescription);
        ...
        $(document).on('submit', '.form-delete-record', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.accreditation.delete') }}",
    ...
    <script>
        var dataTable = initDataTables('table-1', 'loader-accreditation', 'card-accreditation', 'new-record-button', false,
            'Accreditation', "{{ route('admin.accreditation.data') }}",
            [{
                    data: "prodi_name",
                    name: "prodi_name",
