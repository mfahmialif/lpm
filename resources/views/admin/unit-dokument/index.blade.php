...
@section('title', 'Unit Dokumen')
@section('content')
<div class="card" id="card-unit-dokument">
...
<div class="offcanvas offcanvas-end" id="new-record">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title">Tambah Unit Dokumen</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body flex-grow-1">
        <form class="record pt-0 row g-2" id="form-new-record" action="{{ route('admin.unit-dokument.store') }}" method="POST">
            @csrf
            @include('admin.unit-dokument.form')
            <div class="col-sm-12">
...
<div class="offcanvas offcanvas-end" id="edit-record">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title">Edit Unit Dokumen</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body flex-grow-1">
        <form class="record pt-0 row g-2" id="form-edit-record" action="{{ route('admin.unit-dokument.update') }}"
            method="POST">
            @csrf
            @method('PUT')
...
<script>
    ...
    $(document).on('click', '.edit-record-button', function() {
        ...
        $('#form-edit-record [name="posisi"]').val(posisi);
    ...
    $(document).on('submit', '.form-delete-record', function(e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.unit-dokument.delete') }}",
...
<script>
    var dataTable = initDataTables('table-1', 'loader-unit-dokument', 'card-unit-dokument', 'new-record-button', false,
        'Unit Dokumen', "{{ route('admin.unit-dokument.data') }}",
        [{
                data: "nama",
                name: "nama",
