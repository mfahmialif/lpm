...
@section('content')
<div class="card" id="card-accreditation">
...
<div class="offcanvas offcanvas-end" id="new-record">
    ...
    <div class="offcanvas-body flex-grow-1">
        <form class="record pt-0 row g-2" id="form-new-record" action="{{ route('admin.sertifikat.store') }}" method="POST">
            @csrf
            @include('admin.accreditation-sertifikat.form')
            <div class="col-sm-12">
...
<div class="offcanvas offcanvas-end" id="edit-record">
    ...
    <div class="offcanvas-body flex-grow-1">
        <form class="record pt-0 row g-2" id="form-edit-record" action="{{ route('admin.sertifikat.update') }}" method="POST">
            @csrf
            @method('PUT')
...
<script>
    ...
    $(document).on('click', '#new-record-button', function() {
        ...
        $('#form-new-record [name="prodi_id"]').focus();
    });

    $(document).on('click', '.edit-record-button', function() {
        const id = $(this).data('id');
        const link_file = $(this).data('link_file');
        const view = $(this).data('view');
        const prodId = $(this).data('prodi_id');

        $('#form-edit-record [name="id"]').val(id);
        $('#form-edit-record [name="prodi_id"]').val(prodId).change();
        $('#form-edit-record [name="link_file"]').val(link_file);
        $('#form-edit-record [name="view"]').val(view);
        offCanvasEditRecord.show();
        $('#form-edit-record [name="prodi_id"]').focus();
    });
    ...
    $(document).on('submit', '.form-delete-record', function(e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST"
                    , url: "{{ route('admin.sertifikat.delete') }}"
...
<script>
    var dataTable = initDataTables('table-1', 'loader-accreditation', 'card-accreditation', 'new-record-button', false
        , 'Certificate', "{{ route('admin.sertifikat.data') }}"
        , [{
