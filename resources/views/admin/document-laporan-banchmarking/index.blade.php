...
@section('title', 'Dokumen Laporan Benchmarking')
@section('content')
...
<div class="offcanvas offcanvas-end" id="new-record">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title">Tambah Dokumen Laporan Benchmarking</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body flex-grow-1">
        <form class="record pt-0 row g-2" id="form-new-record" action="{{ route('admin.document-laporan-banchmarking.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @include('admin.document-laporan-banchmarking.form')
            <div class="col-sm-12">
...
<div class="offcanvas offcanvas-end" id="edit-record">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title">Edit Dokumen Laporan Benchmarking</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body flex-grow-1">
...
<script>
    ...
    $(document).on('click', '.edit-record-button', function() {
        ...
        $('#form-edit-record').attr('action', "{{ url('admin/document-laporan-banchmarking/update') }}/" + id);
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ url('admin/document-laporan-banchmarking/destroy') }}/" + id,
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
...
<script>
    var dataTable = initDataTables('table-1', 'loader-document', 'card-document', 'new-record-button', false,
        'Dokumen Laporan Benchmarking', "{{ route('admin.document-laporan-banchmarking.data') }}",
        [{
                data: "nama",
                name: "nama",
